...
export const BONUS_GAMES = [
  ...
  // weight:2 — ครูรายงานว่าเล่นจริงหลายรอบแล้วรู้สึกว่าเกมนี้ขึ้นน้อยไป อยากให้ขึ้นบ่อยกว่าเกมอื่น (สนุก/เข้า
  // สังคมกว่า) ดู nextBonusGame() ด้านล่างว่า weight ทำงานยังไง (ค่าเริ่มต้นไม่ใส่ = 1)
  { id: "steal-bonus", name: "ขโมยโบนัส!", emoji: "💰", ready: true, weight: 2 },
...
// สร้าง "ถุงสุ่ม" (bag) จากเกมที่ ready:true ทุกเกม ซ้ำตาม weight ของแต่ละเกม (ไม่ใส่ = 1 เท่า) แล้วสับไพ่ 1
// ครั้งต่อถุง — ยังคงคุณสมบัติเดิมไว้ (ไม่ซ้ำเกมเดียวติดกันในถุงเดียวกันถ้ามีตัวเลือกอื่น เพราะสุ่มมาจาก
// รายการที่ผสมกันแล้วครั้งเดียว) แค่ทำให้เกม weight สูงมีโอกาสถูกจับได้บ่อยกว่าไม่ต้องพึ่งการสุ่มแบบถ่วงน้ำหนัก
// (weighted random) ที่ซับซ้อนกว่านี้
let bag = [];
export function nextBonusGame() {
  const ready = BONUS_GAMES.filter((g) => g.ready);
  if (bag.length === 0) {
    const pool = ready.flatMap((g) => Array(g.weight ?? 1).fill(g));
...
export function runStealBonus(ctx) {
  ...
  const zones = ["tl", "tr", "bl", "br"];
  // v4 (ก.ย. 2026): เพิ่มจาก 12000 → 16000 — ครูรายงานว่า "ยังไม่ทันเลือกเวลาก็หมดแล้ว" ในรอบขโมยโบนัส
  // แม้เห็นเป้าหมายจริงแล้วก็ตาม (คนละปัญหากับ ⏳ ค้างไม่โหลดชื่อ ซึ่งแก้ไปรอบก่อนแล้วด้วย timeout guard)
  // นาฬิกา 12 วิเดิมต้องพอสำหรับ "อ่านชื่อ 4 คน + ตัดสินใจ + ชี้ค้างให้ครบ HOLD_MS (1400ms) ต่อเนื่องไม่สะดุด"
  // — แต่ถ้ามือสั่น/สับสนระหว่างเป้าหมาย 2 คนที่อยู่ใกล้กัน (นาฬิกาค้าง zoneEnterTs รีเซ็ตทุกครั้งที่โซน
  // เปลี่ยน) 12 วิอาจไม่พอจริงๆ โดยเฉพาะเครื่องที่ detection ไม่นิ่ง — เพิ่มเวลาเผื่อให้มากขึ้นอีก 4 วิ
  const DECIDE_MS = 16000;
  ...
  // ครูรายงานเจอเคสจริงที่กล่อง 4 มุมค้างที่ไอคอน ⏳ (placeholder ตอนเริ่ม) ไม่ยอมเปลี่ยนเป็นชื่อจริงเลย —
  // ฟังก์ชัน fetchTargets ที่ ctx ส่งมา (fetchStealTargetsSafe ใน student/app.js) ครอบ timeout ไว้ให้แล้ว
  // (ไม่เกิน 4 วิ ต้อง resolve เสมอ ถือว่า "ไม่มีเป้าหมาย" ถ้าช้ากว่านั้น) ไฟล์นี้เลยไม่ต้องกังวลเรื่อง
  // Firestore ค้างเอง — เรียกตรงๆ ได้เลยเหมือนเดิม
  fetchTargets()
    .then((list) => {
...
export function runCoinCollect(ctx) {
  ...
  const DURATION_MS = 15000;
  const MAX_COINS = 3;
  // v2: ครูรายงานว่าบน Android "เก็บไม่ทัน" — เหรียญขึ้นแล้วตกเร็วเกินไปสำหรับเครื่องที่ตรวจจับช้ากว่า
  // ลด GRAVITY (1.15 -> 0.85) + เพิ่มความแรงยิงขึ้นเริ่มต้น (vy ใน spawnCoin ด้านล่าง) พร้อมกัน สองอย่างนี้
  // ทำงานร่วมกัน: ยิ่งยิงแรงขึ้น + แรงโน้มถ่วงน้อยลง เหรียญจะทั้งขึ้นสูงขึ้น (ตามที่ครูขอ "ให้เก็บได้สูงขึ้น
  // อีกหน่อย") และลอยอยู่บนอากาศนานขึ้นด้วย (เวลาบินรวม = 2×vy/GRAVITY ตามฟิสิกส์ projectile ธรรมดา) — ให้
  // เวลาตรวจจับ/ตอบสนองมากขึ้นโดยไม่ต้องแก้ hitbox หรือกลไกจับเลย
  const GRAVITY = 0.85; // สัดส่วนจอ/วินาที² — ค่อยๆ ดึง vy กลับลงจนเหรียญเริ่มตกหลังขึ้นถึงจุดสูงสุด
  const CATCH_RX = 0.14;
  const CATCH_RY = 0.11;
  const MARGIN_X = 0.1;
  ...
  function spawnCoin() {
    ...
    coins.push({
      ...
      vy: -randRange(0.75, 1.05), // ติดลบ = ขึ้น (y น้อยลง) ความแรงสุ่มให้เหรียญขึ้นสูงไม่เท่ากันทุกครั้ง —
      // เพิ่มจาก 0.55-0.85 คู่กับ GRAVITY ที่ลดลง (ดูคอมเมนต์เหนือ GRAVITY ด้านบน) ให้ทั้งขึ้นสูงขึ้นและ
      // ลอยนานขึ้นพร้อมกัน
    });
  }
